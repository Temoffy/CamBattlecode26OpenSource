...
class BaseBuildingMixin (BuilderBase):
  ...
  def runConveyorLaying(self, ct: Controller) -> None:
    ...
    for dir in OCT_DIRECTIONS:
      ...
      if type == EntityType.BRIDGE:
        if ct.get_bridge_target(building) == myPos:
          if ct.can_move(dir):
            print("follow bridge move")
            self.newPosition = False
            ct.move(dir)
            return
    ...
    forbidList:  list[Position] = []
    forbidList.append(myPos)
    for dir, pos in possibleMoves:
      if dir in QUAD_DIRECTIONS:
        forbidList.append(pos)
    for dir, pos in possibleMoves:
      if dir in QUAD_DIRECTIONS:
        forbidList.append(pos.add(dir))
    for dir, pos in possibleMoves:
      if dir in self.moveScoring:
        continue
      score = sensing.scoreOctNocrossReachableTiles(pos, ct, forbiddenPosList=forbidList)
      print("dir:",dir,"score:",score)
      if dir not in QUAD_DIRECTIONS:
        score = score - 2
  ...
  def __conveyMove(self, dir:Direction, convDir:Direction, ct:Controller):
    ...
    if dir in QUAD_DIRECTIONS:
      ...
      if ct.can_build_bridge(pos, ct.get_position()):
        ct.build_bridge(pos, ct.get_position())
    if ct.can_move(dir):
      self.newPosition = False
      ct.move(dir)
  ...
  def __tryBuildSentry(self, ct: Controller):
    ...
    for dir in OCT_DIRECTIONS:
      ...
      if not posIsInbounds(ct, testPos):
        continue
      if ct.get_tile_env(testPos) == Environment.WALL or not ct.can_build_sentinel(testPos, testPos.direction_to(aimingPos)):
        continue
  ...
  def runReturnToCore(self, ct: Controller):
    ...
    if entityUnderId is not None and ct.get_team() == ct.get_team(entityUnderId):
      if entityUnderType == EntityType.CONVEYOR:
        ...
        self.revDir = moveDir
      if entityUnderType == EntityType.BRIDGE:
        moveDir = mypos.direction_to(ct.get_bridge_target(entityUnderId))
        self.revDir = moveDir
  ...
  def runFindNewBranch(self, ct:Controller):
    ...
    buildingUnderType = ct.get_entity_type(buildingUnder)
    if buildingUnderType == EntityType.CORE:
      self.mode = Modes.RETURN_TO_CORE
      return
    
    if buildingUnder == None or buildingUnderType not in [EntityType.CONVEYOR,EntityType.ARMOURED_CONVEYOR,EntityType.BRIDGE]:
    ...
    if len(possibleDirs) == 0:
      ...
      if buildingUnderType != EntityType.BRIDGE:
        backDir = ct.get_direction(buildingUnder)
      else:
        backDir = myPos.direction_to(ct.get_bridge_target(buildingUnder))
      ...
      if ct.can_move(dir):
        ct.move(dir)
        return
      if ct.can_build_road(myPos.add(dir)):
        ct.build_road(myPos.add(dir))
      if ct.can_move(dir):
        ct.move(dir)
      return
    ...
    #and fix it if so
    if buildingUnderType != EntityType.BRIDGE:
      priorityDir = ct.get_direction(buildingUnder)
